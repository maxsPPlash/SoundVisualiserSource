...
static const char *video_path = "videoplayback.avi";
...
bool VideoVis::Initialize(HINSTANCE hInstance, std::string window_title, std::string window_class, int width, int height) {
	...
	OpenCVVideoStream *vd = new OpenCVVideoStream();
	vd->Initialise(video_path);
	video_stream = vd;
...
void VideoVis::Update() {
	bool snd_updated = snd_stream->Update(time);

	video_stream->Update(time);
