...
Composite::Composite() {
	...
	for (int i = 0, sz = sizeof(bass_time_data)/sizeof(bass_time_data[0]); i < sz; ++i) {
		bass_time_data[i] = 0;
	}
}
...
bool Composite::Initialize(HINSTANCE hInstance, std::string window_title, std::string window_class, int width, int height) {
	...
	bass_data_tex = new ShaderDynTexture(bass_time_data, 1, 512, DT_U8R, 1);
	dyn_textures.push_back(bass_data_tex);
	...
	return Engine::Initialize(hInstance, window_title, window_class, width, height, recorder, L"composite", &scb, dyn_textures, stat_textures);
}
...
//			float new_data = clamp((fft_prev[i] + fft_res[i]) / 2 * 255., 0., 255.);
		...
		cbuffer.bass_coef /= bass_samples_cnt*8.;

		unsigned char bass_new_val = (int(bass_time_data[0]) + int(clamp(cbuffer.bass_coef, 0., 1.) * 255)) / 2;

		for (int i = sizeof(bass_time_data)/sizeof(bass_time_data[0]) - 1; i >= 0; --i) {
			bass_time_data[i+1] = bass_time_data[i];
		}
		if (bass_time_data[1] >= bass_new_val)
			bass_time_data[0] = bass_new_val;
		else
			bass_time_data[0] -= min(5, bass_time_data[0] - bass_new_val);
